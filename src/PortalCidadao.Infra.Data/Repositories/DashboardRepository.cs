...
namespace PortalCidadao.Infra.Data.Repositories
{
    public class DashboardRepository : IDashboardRepository
    {
        ...
        public async Task<DashboardAbertos> ObterDashboardAbertos(string mes)
        {
            var mesParam = string.IsNullOrEmpty(mes) ? default : mes;
            const string sql = @"
                    SELECT COUNT(P.Id) AS QtdPostagens,
            ...
            return await _dbConnection.QueryFirstAsync<DashboardAbertos>(sql, new {mesParam});
        }
         ...
         public async Task<int> ObterTotalAbertos()
         {
             const string sql = @"
                    SELECT COUNT(P.Id) AS QtdPostagens
                    FROM Postagem P                     
